...
function ceres.openObjectData(data, kind)
    local store = {}
    store.__handle = objdata.openStore(data, kind)

    setmetatable(store, objStoreMeta)
    return store
end

local objStoreFuncs = {
    all = function(self)
        return self.__handle:getObjects()
    end
}

objStoreMeta.__index = function(self, k)
    if objStoreFuncs[k] then
        return objStoreFuncs[k]
    elseif #k == 4 then
        local handle = self.__handle:getObject(k)
        if handle ~= nil then
            local proxy = { __handle = handle }
            setmetatable(proxy, objMeta)
            return proxy
        end
    else
        return nil
...
objStoreMeta.__newindex = function(self, k, v)
    if type(k) ~= "string" or #k ~= 4 then
        error("key must be a 4-char string")
    end

    if getmetatable(v) == objMeta then
        local clone = v:clone(k)
        self.__handle:addObject(clone)
    else
        error("cannot set a non-object value on an object store")
    end
end
...
objMeta.__index = function(self, k)
    if objFuncs[k] then
        return objFuncs[k]
    else
        local field, level = self.__handle:translateField(k)

        if field ~= nil then
            if level ~= nil then
                return self.__handle:getLevelField(field, level)
            else
                return self.__handle:getField(field)
            end
        end

        error("no such field " .. tostring(k) .. " on object")
    end
end

objMeta.__newindex = function(self, k, v)
    local field, level = self.__handle:translateField(k)

    if field ~= nil then
        if level ~= nil then
            self.__handle:setLevelField(field, level, v)
        else
            self.__handle:setField(field, v)
        end
