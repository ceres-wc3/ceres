    ...
    fn index<'lua>(
        ...
        if let Ok(key) = LuaString::from_lua(key.clone(), ctx) {
            ...
            match key {
                ...
                b"type" => return Ok(object.kind().to_typestr().to_lua(ctx)?),
                _ => {}
            }
        }
